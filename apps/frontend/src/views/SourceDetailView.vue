...
import {
  useSourceStore,
  type SourcePage,
} from "../features/sources/source.store";
import {
  ArrowLeft,
  Database,
  FileText,
...
onMounted(async () => {
  ...
  if (id) {
    ...
    // Poll if active
    if (
      source.value?.status === "in_progress" ||
      source.value?.status === "pending" ||
      source.value?.status === "processing"
    ) {
...
const loadMoreChunks = async () => {
  ...
  const newChunks = await store.fetchChunks(
    source.value.id,
    currentLength,
    100,
  );
...
<template>
  <div
    class="space-y-6 w-full p-6 lg:p-10 animate-in fade-in duration-500 h-[calc(100vh-4rem)] flex flex-col"
  >
    <!-- Header -->
    <div
      class="flex items-center space-x-4 border-b border-border pb-4 flex-shrink-0"
    >
      <Button
        variant="ghost"
        size="icon"
    ...
    >
      <div
        class="animate-spin h-10 w-10 border-2 border-primary border-t-transparent rounded-full"
      />
    ...
    >
      ...
      <div class="md:col-span-1 flex flex-col gap-6 overflow-y-auto pr-2">
        <Card
          class="bg-card/50 backdrop-blur-sm border-border shadow-sm flex-shrink-0"
        >
          ...
          <CardContent class="space-y-4 pt-4">
            <div class="space-y-1">
              ...
              <div class="flex items-center gap-2">
                ...
                >
                  {{ source.url }}
                  <ExternalLink class="inline h-3 w-3 ml-0.5 mb-0.5" />
                </a>
                <span
                  v-else
            ...
            <div
              v-if="
                source.max_depth > 0 ||
                  (source.exclusions && source.exclusions.length > 0)
              "
            ...
            >
              ...
              >
                ...
                >Crawl Depth</span>
                <span
                  class="text-sm font-mono bg-secondary px-2 py-0.5 rounded"
                >{{ source.max_depth }}</span>
              </div>
              <div
      ...
      <div
        class="md:col-span-2 flex flex-col h-full min-h-[500px] border border-border rounded-xl bg-card/30 backdrop-blur-sm shadow-sm overflow-hidden"
      >
        <!-- Header -->
        <div
          class="flex items-center justify-between px-4 py-3 border-b border-border bg-secondary/10"
        >
        ...
        <div class="flex flex-1 min-h-0">
          ...
          <div
            class="w-1/3 border-r border-border overflow-y-auto flex flex-col bg-background/20"
          >
            <div
            ...
            <button
              ...
              :class="{
                'bg-primary/10 border-l-2 border-l-primary':
                  selectedChunk === chunk,
                'border-l-2 border-l-transparent': selectedChunk !== chunk,
              }"
              @click="selectedChunk = chunk"
            >
              <div class="flex items-center justify-between mb-1">
                <span
                  class="font-mono text-xs text-muted-foreground flex items-center gap-1"
                >
                ...
                <div class="flex items-center gap-1">
                  <Badge
                    v-if="chunk.type"
                    variant="outline"
                    class="text-[9px] px-1 py-0 h-4 uppercase"
                  >
                    {{ chunk.type }}
                  </Badge>
                  <span
                    class="text-[10px] text-muted-foreground bg-secondary px-1.5 py-0.5 rounded"
                  >
              ...
              <div
                v-if="chunk.title"
                class="text-xs font-semibold truncate text-foreground/80 mb-0.5"
              >
                {{ chunk.title }}
              </div>
            ...
            <div
              v-if="source.chunks && source.chunks.length < source.total_chunks"
              class="p-3 text-center border-b border-border/50"
            >
              <Button
                variant="outline"
                size="sm"
              ...
              >
                <span
                  v-if="isLoadingMore"
                  class="animate-spin mr-2 h-3 w-3 border-2 border-primary border-t-transparent rounded-full"
                />
                Load More ({{ source.chunks.length }} /
                {{ source.total_chunks }})
              </Button>
            </div>
          </div>
          ...
          <div class="flex-1 overflow-y-auto bg-card/10 p-6">
            ...
            >
              <div
                class="flex items-start justify-between border-b border-border/50 pb-4"
              >
                <div class="space-y-1">
                  <div class="flex items-center gap-2">
                    <h4
                  ...
                  >
                    ...
                    <span
                      ...
                      :title="selectedChunk.source_url"
                    >
              ...
              <div class="space-y-2">
                <label
                  class="text-xs uppercase tracking-wider font-semibold text-muted-foreground flex items-center gap-2"
                >
                ...
                <div class="relative group">
                  ...
                  <pre
                    class="whitespace-pre-wrap font-mono text-sm leading-relaxed bg-background/50 p-4 rounded-lg border border-border text-foreground/90 overflow-x-auto"
                  >{{ selectedChunk.content }}</pre>
                </div>
              </div>
            </div>
