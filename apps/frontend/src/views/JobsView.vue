  ...
  <div class="space-y-6 w-full p-6 lg:p-10">
    <div class="flex items-center justify-between">
      ...
      <Button
        ...
        @click="jobStore.fetchFailedJobs"
      >
        <RefreshCw
          class="mr-2 h-4 w-4"
          :class="{ 'animate-spin': jobStore.isLoading }"
        />
        Refresh Logs
      </Button>
    </div>
    ...
    <div
      v-if="jobStore.jobs.length === 0 && !jobStore.isLoading"
      class="flex flex-col items-center justify-center p-12 bg-card/50 border border-border rounded-lg border-dashed"
    >
      ...
      <p class="text-muted-foreground">
        No ingestion failures detected in the logs.
      </p>
    </div>

    <div
      v-else
      class="rounded-md border border-border overflow-hidden bg-card/50 backdrop-blur-sm"
    >
        ...
        <table class="w-full text-sm text-left">
          ...
          <tbody class="divide-y divide-border">
            ...
            >
              ...
              <td
                class="px-6 py-4 font-mono text-muted-foreground whitespace-nowrap"
              >
              ...
              <td class="px-6 py-4">
                <div class="flex items-start gap-2 max-w-md">
                  ...
                  />
                  <code
                    class="text-xs text-destructive/90 font-mono bg-destructive/10 px-1 py-0.5 rounded break-all line-clamp-2 hover:line-clamp-none transition-all"
                  >
                    {{ job.error }}
                  </code>
                </div>
              ...
              <td class="px-6 py-4 text-right">
                ...
                >
                  <RefreshCw
                    class="w-4 h-4 text-primary hover:text-primary/80"
                  />
